...
It's a bit difficult to tell what group each participant is in with their IDs combined with their grouping. Create a new column called "GROUP" based on the letter assignment for IDs. Makes ure to following naming convetion above.
...
On the first page are some very useful syntax to create *awesome* pdfs. Practice a few of them below. The fun part (or frustrating part) is the repeated knitting to see if your intention worked out.
